...
![Cover Banner](task-tamer/assets/cover_page_banner.png)
...
![Architecture Diagram](task-tamer/assets/architecture_diagram.png)
...
├── .env                     # Secrets (NOT committed — add your key here)
├── .gitignore
...
```bash
git clone https://github.com/charanemandi26-arch/task-tamer.git
cd task-tamer/task-tamer
uv sync
```

### 2. Configure Environment

Create a `.env` file inside the `task-tamer/` folder:
...
```
**Expected:** `security_checkpoint` detects injection keyword → routes to `SECURITY ALERT: Access Denied.` (orchestrator is never invoked)
...
| `rerun_on_resume` ValueError | Ensure `@node(rerun_on_resume=True)` on any node using `ctx.run_node()` or `RequestInput` |
